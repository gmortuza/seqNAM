...
Error distribution
"""
cursor.execute("select r1_count, r2_count, probable_err, probable_err_in_primer from sequences")
...
total_sequences_in_r1 = sum([k * v for k, v in r1_frequency.items()])
...
total_sequences_in_r2 = sum([k * v for k, v in r2_frequency.items()])
print(f"r2 total sequences==> {total_sequences_in_r2}")
# TODO: replace 1 with total_sequences_in_r2
# r2_frequency = {k: round(v / 1, 2) * 100 for k, v in r2_frequency.items()}
...
print(f"r1_gt_frequency==>{total_gt_frequency}")
print(f"r2_gt_frequency==>{total_gt_frequency}")
connection.close()
